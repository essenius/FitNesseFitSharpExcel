...
namespace ExcelFixtureTest
{
    ...
    public class ExcelQueryTest
    {
        ...
        [TestMethod]
        public void ExcelQueryFixtureDocumentationTest()
        {
            var doc = ExcelQuery.FixtureDocumentation;
            Assert.IsTrue(doc.Count > 0, "there are items");
            Assert.IsTrue(doc.ContainsKey(nameof(ExcelQuery.Query)), "there is an entry for Query");
        }
    }
}